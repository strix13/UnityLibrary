...
public class PCMission_EnemyBoss2 : PCMission_Enemy
{
	...
	public enum EBossAnimationName
	{
		Dead,
		Transform,
	...
	protected override void OnEnableObject()
	{
		...
		_pAnimator.DoPlayAnimation( EBossAnimationName.GunTurn, false );
		// TestCode
	...
	protected override void OnDecreaseHP( float fHPPercentage )
	{
		if(fHPPercentage <= 0f)
		{
			StartCoroutine( CoDelayOnDead( 2f ) );
		}
		else if(_bIsTransformRobot == false && fHPPercentage <= 0.5f)
		{
			ProcTransformRobot();
		}
	...
	private void PCMission_Boss_1_p_EVENT_OnArriveWayPoint( string obj )
	{
		...
		if(_bIsTransformRobot)
		{
			...
			if (obj == EWayPoint.LeftUp.ToString_GarbageSafe() || obj == EWayPoint.RightUp.ToString_GarbageSafe())
			{
				...
				if (iRand == 0)
				{
					_pEnemyMove.EventSetEnemyMoveSpeed( 0.5f );
	...
	private void ProcTransformRobot()
	{
		...
		EventStopAllBulletMuzzle();
		_pEnemyMove.EventSetEnemyMoveSpeed( 0f );
		_pAnimator.DoPlayAnimation( EBossAnimationName.Transform, OnFinishTransformAnimation );
	}

	private void OnFinishTransformAnimation()
	{
		_pAnimator.DoPlayAnimation( EBossAnimationName.Idle );
		EventSetColliderOnOff( true );
		_pEnemyMove.EventSetEnemyMoveSpeed( 1f );
		DoRotateAngle( Vector3.zero );
	}

	private IEnumerator CoDelayOnDead(float fDelaySec)
	{
		_pEnemyMove.EventSetEnemyMoveSpeed( 0f );
		_pAnimator.DoPlayAnimation( EBossAnimationName.Dead );

		yield return new WaitForSeconds( fDelaySec );

		PCManagerInMission.instance.DoShowSpaceStation();
		base.OnDecreaseHP( 0f );
	}
}
