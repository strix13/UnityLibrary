...
#define const_iArraySize 20
	...
	float3 _DropInfo[const_iArraySize];
	...
	float allwave(float2 position)
	{
		...
		float fWaveTotal = 0.0f;
		for(int i = 0; i < const_iArraySize; i++)
			fWaveTotal += wave(position, _DropInfo[i].xy, _DropInfo[i].z);
		return fWaveTotal;
	}
