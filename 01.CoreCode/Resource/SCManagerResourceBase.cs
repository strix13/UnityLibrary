    ...
    where RESOURCE : UnityEngine.Object
{
    ...
    static public RESOURCE DoGetResource_Origin(ENUM_RESOURCE_NAME eResourceName)
    {
    ...
    protected void InitResourceOrigin()
    {
        ...
        for (int i = 0; i < arrResources.Length; i++)
        {
            ...
            {
                bSuccess = false;
                Debug.LogWarning(string.Format("{0}에{1} 이 존재하지 않습니다.", typeof(ENUM_RESOURCE_NAME).ToString(), arrResources[i].name));
            }
    ...
    {
        RESOURCE[] arrResources = Resources.LoadAll<RESOURCE>(_strResourceLocalPath + "/");
        ENUM_RESOURCE_NAME[] arrResourceName = PrimitiveHelper.DoGetEnumType<ENUM_RESOURCE_NAME>();
